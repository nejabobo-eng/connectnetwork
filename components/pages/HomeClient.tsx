...
export default function HomeClient() {
  ...
  useEffect(() => {
    fetch('/api/products').then(async response => {
      ...
      setProducts(Array.isArray(body.products) ? body.products.filter((product: Product) => product.sponsored) : [])
  ...
  return <main>
    ...
    <section className="bg-slate-50 py-16"><div className="container-section"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="font-semibold text-green">SPONSORED</p><h2 className="section-title mt-1">Products in the spotlight</h2><p className="mt-2 text-slate-600">Featured placements from suppliers who want to reach more shoppers.</p></div><Link href="/promotion" className="btn btn-ghost">Promote a product</Link></div>{loading ? <p className="mt-8 text-slate-600">Loading sponsored products…</p> : products.length ? <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{products.map(product => <SponsoredProductCard key={product.id} product={product} />)}</div> : <div className="mt-8 rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-slate-600">Sponsored products will appear here soon. <Link href="/products" className="font-semibold text-primary hover:underline">Browse all products</Link>.</div>}</div></section>
...
function SponsoredProductCard({ product }: { product: Product }) {
  const href = `/products/${product.slug || product.id}`
  return <article className="card group overflow-hidden"><div className="relative h-56 overflow-hidden bg-gradient-to-br from-navy to-blue-700">{product.image_url ? <img src={product.image_url} alt={product.name} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /> : <span className="absolute bottom-5 left-5 text-lg font-bold text-white">{product.name}</span>}<span className="absolute left-3 top-3 rounded-full bg-slate-950 px-3 py-1 text-xs font-bold text-white">SPONSORED</span></div><div className="p-5"><p className="text-sm text-slate-500">{product.category || 'Marketplace'}</p><h3 className="mt-1 text-lg font-bold">{product.name}</h3><div className="mt-4 flex items-center justify-between gap-3"><span className="font-bold text-primary">{price(product)}</span><Link href={href} className="rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-white">View product</Link></div></div></article>
}
