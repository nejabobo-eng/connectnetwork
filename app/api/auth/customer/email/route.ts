...
export async function POST(request: Request) {
  ...
  if (mode === 'sign-up' && (!firstName || !lastName)) return NextResponse.json({ error: 'Enter your first name and surname to create an account.' }, { status: 400 })
  ...
  const { data, error } = mode === 'sign-up'
    ? await supabase.auth.signUp({ email, password, options: { data: { first_name: firstName, last_name: lastName, full_name: `${firstName} ${lastName}` } } })
