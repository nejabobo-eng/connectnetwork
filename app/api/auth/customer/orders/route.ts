...
export async function GET() {
  try {
    ...
    const orders = await adminRequest(`orders?customer_email=eq.${encodeURIComponent(email)}&status=in.(paid,processing,fulfilled,refunded)&order=created_at.desc&select=id,status,total_cents,currency,delivery_status,created_at`)
